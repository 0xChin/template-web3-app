...
function getStatusIcon(status: string, error_status: string) {
  const pending = !['Executed', 'CompletedFast', 'CompletedSlow'].includes(status as 'Executed' | 'CompletedFast' | 'CompletedSlow')
  const errored = error_status === 'LowRelayerFee' && ['XCalled', 'Reconciled'].includes(status)

  if (errored) {
    return (
      <div className="flex flex-col items-center justify-center">
        <IoWarning className="3xl:w-10 3xl:h-10 text-red-500 dark:text-red-400" size={32} />
...
export function Transfer({ isMainnet, transfer }: TransferProps) {
  ...
  const originDecimals = originAsset ? findDecimals(originAsset, transfer.origin_transacting_asset) : undefined

  const destinationChain = findChain(chains, transfer.destination_domain)
  const destinationAsset = findAsset(assets, transfer.destination_transacting_asset)
  const destinationDecimals = destinationAsset ? findDecimals(destinationAsset, transfer.destination_transacting_asset) : undefined
        ...
        <div className="flex items-center justify-between space-x-2">
          <div className="flex items-center space-x-1.5">
            <Image
              ...
              height={20}
              src={originChain?.image || '/integrations/connext/logos/chains/gnosis.png'}
              width={20}
            />
            <span className="3xl:text-xl text-xs font-medium">{originChain?.name}</span>
          </div>
          <div className="flex items-center justify-end space-x-1.5">
            <Image
              ...
              className="3xl:w-6 3xl:h-6 rounded-full"
              height={20}
              src={destinationChain?.image || '/integrations/connext/logos/chains/gnosis.png'}
              width={20}
            />
            <span className="3xl:text-xl text-xs font-medium">{destinationChain?.name}</span>
          </div>
        </div>
        <div className="my-4 flex items-start justify-between space-x-2">
          <div className="flex flex-col space-y-1.5">
            <span className="3xl:text-xl text-sm">{calculateAmount(transfer.origin_transacting_amount, originDecimals || 18)}</span>
            <div className="flex items-center justify-start space-x-1">
              <Image
                alt={`${originAsset?.name || ''}`}
                className="3xl:w-5 3xl:h-5 rounded-full"
                height={16}
                src={originAsset?.image || '/integrations/connext/logos/chains/gnosis.png'}
                width={16}
              />
              <span className="3xl:text-xl text-xs font-medium">{originAsset?.symbol}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end space-y-1.5">
            ...
            <div className="flex items-center justify-center space-x-1">
              <Image
                ...
                className="3xl:w-5 3xl:h-5 rounded-full"
                height={16}
                src={destinationAsset?.image || '/integrations/connext/logos/chains/gnosis.png'}
                width={16}
              />
              <span className="3xl:text-xl text-xs font-medium">{destinationAsset?.symbol}</span>
            </div>
          </div>
        </div>
